          ...
          bslib::card(
            ...
            bslib::card_body(
              shiny::tags$ul(
                ...
                shiny::tags$li(shiny::tags$b("Ambiguity Resolution"), " — The results table contains one row per unique scientificName. When a match is uncertain, a manual selection is required to confirm the correct taxon."),
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    shiny::observe({
      ...
      msg2 <- paste0(
        ...
        if (length(aurora_cols) > 0) paste(aurora_cols, collapse = ", ") else ""
      )
    ...
    output$keep_cols_ui <- shiny::renderUI({
      ...
      selected <- if (identical(input$tax_db, "worms")) {
        unique(c("scientificName", "scientificNameID", intersect(final_match_cols_default, unname(choices))))
      } else {
        unique(c("scientificName", intersect(final_match_cols_default, unname(choices))))
      }
    ...
    shiny::observeEvent(input$keep_output_cols, {
      ...
      if (!identical(sort(sel), sort(current_sel))) {
        shiny::updateCheckboxGroupInput(session, "keep_output_cols", selected = sel)
      }
    }, ignoreInit = TRUE)
    ...
    normalize_gbif_candidates <- function(x) {
      ...
      out <- data.frame(
        ...
        status = {
          ...
          if (all(is.na(st) | !nzchar(trimws(st)))) safe_col(df, "taxonomicStatus") else st
        },
      ...
      out$scientificName[!nzchar(trimws(out$scientificName))] <- out$canonicalName[!nzchar(trimws(out$scientificName))]
      out$acceptedScientificName[!nzchar(trimws(out$acceptedScientificName))] <- out$scientificName[!nzchar(trimws(out$acceptedScientificName))]
    ...
    match_one_gbif <- function(input_name, normalised_query, auto_apply_unique = TRUE) {
      ...
      status_out <- if (!is.na(top_conf) && top_conf >= 90) "MATCHED_AUTO" else "AMBIGUOUS"
      selected_id <- if (identical(status_out, "MATCHED_AUTO")) top_usage else NA_character_
    ...
    candidate_tree <- candidate_tree[!is.na(candidate_tree) & nzchar(trimws(candidate_tree))]
    ...
    other_rows_message <- if (!is.na(obs_count)) {
      paste0(
        ...
        obs_count, " observation(s) in the dataset",
        if (other_obs_count > 0) paste0(" (", other_obs_count, " other entr", ifelse(other_obs_count == 1, "y", "ies"), ").") else "."
      )
    } else {
      paste0("Could not determine how many dataset observations share scientificName input value ('", nm, "').")
    }
            ...
            if (lookup$taxonMatchStatus[i] == "AMBIGUOUS") {
              if (identical(input$tax_db, "worms")) {
                ...
                add_issue(
                  NA_integer_, src, "taxonomy_ambiguous", "WARNING",
                  paste0(
                    "GBIF Backbone returned a lower-confidence match for: '", nm,
                    "' (query='", q, "', confidence=",
    ...
    obs_list[[j]] <- local({
      ...
      shiny::observeEvent(input[[input_name]], {
        ...
        if (isTRUE(rv$suppress_choice_popup)) {
          ...
          add_issue(
            NA_integer_, src, "taxonomy_manual_resolution_same_inputName",
            "INFO",
            paste0(
              "Choice applied to ",
              length(same_rows),
              " row(s) with inputName='", nm_same, "'."
            )
          )
        } else {
  ...
  rv$pending_choice <- NULL
    ...
    if (is.data.frame(df) && !is.null(src) && src %in% names(df)) {
      ...
      q_by_obs <- vapply(
        src_vals,
        function(x) {
          idx <- match(x, rv$lookup$inputName)
          if (is.na(idx)) return(NA_character_)
          rv$lookup$normalisedQuery[idx]
        },
        character(1)
      )

      obs_count <- sum(!is.na(q_by_obs) & q_by_obs == q)
    }
    ...
    add_issue(
      ...
      paste0(
        ...
        length(same_rows),
        " lookup row(s) with scientificName='", q, "'."
      )
    )
    ...
    compute_out <- function() {
      ...
      lk_col <- function(col) {
        ...
        ok <- !is.na(map_idx) & map_idx >= 1 & map_idx <= nrow(rv$lookup) & (col %in% names(rv$lookup))
      ...
      fill_ok_blank <- status_group == "MATCHED" &
        ...
        !is.na(fill_val) & nzchar(fill_val)
      
      out$scientificName[fill_ok_blank] <- fill_val[fill_ok_blank]
      
      if (isTRUE(replace_scientificName)) {
        fill_ok <- status_group == "MATCHED" & !is.na(fill_val) & nzchar(fill_val)
    ...
    finalize_apply <- function() {
      ...
      shiny::showNotification(
        paste0(
          ...
          nrow(out), " row(s) kept and ",
          max(0, discarded_n), " row(s) discarded."
        ),
        type = "message",
        duration = 6
      )
